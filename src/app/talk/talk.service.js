(function(){ 'use strict';
    ...
    angular.module( 'dtr4' ).factory( 'Talk', TalkService );

    TalkService.$inject = [ '$q', '$http', '$sce', '$sanitize', 'SharedFunctions' ];

    function TalkService( $q, $http, $sce, $sanitize, SharedFunctions ){

        var _this = this;
        _this.getQuickie = getQuickie;
        _this.getPosts = getPosts;
        _this.getOlderPosts = getOlderPosts;
        _this.getPostsByUsername = getPostsByUsername;
        _this.getPostsByHashtag = getPostsByHashtag;
        _this.addPost = addPost;
        _this.delPost = delPost;
        _this.getHashtagsList = getHashtagsList;
        _this.getUsernamesList = getUsernamesList;
        _this.completePost = completePost;
        _this.countNewPosts = countNewPosts;
        _this.resetNewPostsTimestamp = resetNewPostsTimestamp;
        _this.makeHtmlText = makeHtmlText;
        ...
        function completePost( data ) {
            data['user'] = SharedFunctions.complete_user_pnasl( data['user'] );
            data['created_delta'] = SharedFunctions.get_time_delta( data['created'] );
        ...
        function getHashtagsList( ){
            ...
            if ( !hashtags_timestamp || SharedFunctions.get_time_delta_seconds( hashtags_timestamp ) > 60*20 ) {
        ...
        function getUsernamesList( ){
            ...
            if ( !usernames_timestamp || SharedFunctions.get_time_delta_seconds( usernames_timestamp ) > 60*20 ){
