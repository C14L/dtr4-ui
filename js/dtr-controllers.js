...
app.controller( 'SearchController', 
   [ '$scope', 'Authuser', 'Cities', 'Search', 'appBarTitle',
        function( $scope, Authuser, Cities, Search, appBarTitle ){
            ...
            log( '--- SearchController: $scope.search:' ); log( $scope.search );
            ...
            $scope.updateResults = function( page ){
                ...
                Search.getResults( page ).then(
                    function( data ){
                        $scope.isLoadingMore = false;
                        $scope.statusMsg = ( data.length < 1 ) ? 'empty' : '';
                        $scope.nextPage = Search.getPage() + 1;

                        data.forEach(function( row, i ){
                            row = complete_user_pnasl( row );
                            $scope.userlist.push( row ); // append new results
                        });
                    },
                    function( err ){
...
app.controller( 'TalkController', 
    [ '$scope', '$http', '$interval', '$sce', '$sanitize',
      '$routeParams', 'Talk', 'appBarTitle',
        function( $scope, $http, $interval, $sce, $sanitize,
                  $routeParams, Talk, appBarTitle ){
            ...
            $scope.groupNames = { 'all': $scope.tr('by all'), 
            ...
            $scope.delPost = function( post_id ){
                ...
                Talk.delPost( post_id ).then( function( data ){ 
                    //pass
                }, function( err ){
            ...
            $scope.addPost = function(){ 
                ...
                Talk.addPost( $scope.posttext, $scope.group ).then( function( post ){
                    ...
                    log( '!!! TalkController.$scope.addPost() !!! ERROR: ' + err );
                } );
            }
            ...
            $scope.getOlderPosts = function( ){
                ...
                Talk.getOlderPosts( $scope.group ).then( function( data ){
                    $scope.posts = data;
                    $scope.isLoadingMore = false;
                }, function( err ){
            ...
            $scope.getPosts = function( ){
                ...
                Talk.getPosts( $scope.group ).then( function( data ){
                    $scope.posts = data;
                    $scope.isLoadingMore = false;
                }, function( err ){
                    $scope.statusMsg = 'loading-error';
            ...
            $scope.getPostsByHashtag = function( before ){
                ...
                Talk.getPostsByHashtag( $scope.hashtag, !!before ).then( function( data ){
            ...
            $scope.getPostsByUsername = function( before ){
                ...
                Talk.getPostsByUsername( $scope.username, !!before ).then( function( data ){
